...
set rc [catch {
  ...
  elaborate -top {al_temac_example_design}
  export_db {temac_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../mac_pins.adc
  optimize_rtl
  report_area -file temac_rtl.area
  export_db {temac_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  ...
  optimize_gate -maparea temac_gate.area
  legalize_phy_inst
  export_db {temac_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
